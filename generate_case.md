...
sudo tar cvf cpd-5.3.1-offline-case.tar offline
sudo gzip cpd-5.3.1-offline-case.tar
sudo mv cpd-5.3.1-offline-case.tar.gz ../../../cpd-silent-install
```
...
```shell
mkdir work
mv cpd-5.3.1-offline-case.tar.gz work
cd work
split -d -b 1000000 cpd-5.3.1-offline-case.tar.gz case-

cd ..
for i in `ls work/case-*`
do 
  filename=$(echo $i | cut -f 2 -d /)
  oc create cm $filename --from-file=$filename=work/$filename --dry-run=client -o yaml > $filename.yaml
done
```
